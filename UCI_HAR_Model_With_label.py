...
class ImageGenerator:
    ...
    def save_fig(fig, out_path: Path, dpi=DPI, resize_to=RESIZE_TO):
        """
        Save figure to PNG — publication-ready.
        tight_layout() ensures titles, axis labels, and colorbars
        are never clipped.  Saves at DPI=300.
        """
        plt.tight_layout()
    ...
    def scalogram(sig, fs, scales, wavelet, out_path: Path,
        ...
        Includes descriptive title, X/Y axis labels, ticks,
        colorbar ('Magnitude'), tight_layout(), DPI=300.
        """
        ...
        # Title using sample ID from caller
        ax.set_title(title if title else 'Scalogram',
    ...
    def spectrogram(sig, fs, nperseg, noverlap, nfft, out_path: Path,
                    title='', t_offset=0.0, vmin=None, vmax=None):
        """
        STFT-based spectrogram — publication-ready.
        Includes descriptive title, X/Y axis labels, ticks,
        colorbar ('Power (dB)'), tight_layout(), DPI=300.
        """
        ...
        pcm = ax.pcolormesh(t_axis, f, Sxx_db,
    ...
    def kurtogram(sig, fs, scales, window_samples, step, out_path: Path,
        ...
        Includes descriptive title, X/Y axis labels, ticks,
        colorbar ('Excess Kurtosis'), tight_layout(), DPI=300.
        """
        ...
        # Title using sample ID from caller
        ax.set_title(title if title else 'Kurtogram',
                     fontsize=14, fontweight='bold', pad=10)
    ...
    def xyz_with_original(x_norm, y_norm, z_norm, original_signal, fs,
        ...
        Includes figure title (sample ID), individual subplot titles,
        Y-axis label on every subplot, X-axis label on bottom subplot only,
        ticks enabled, clean spines, tight_layout(), DPI=300.
        """
        ...
        # Overall figure title using sample ID passed in title param
        fig.suptitle(title if title else 'XYZ + Resultant',
                     fontsize=14, fontweight='bold', y=1.01)

        subplot_info = [
            (axes[0], x,    'tab:blue',  'X-axis Signal'),
            (axes[1], y,    'tab:orange', 'Y-axis Signal'),
            (axes[2], z,    'tab:green',  'Z-axis Signal'),
            (axes[3], orig, 'black',      'Resultant Magnitude'),
        ]

        for i, (ax, data, color, sub_title) in enumerate(subplot_info):
            lw = 1.5 if i < 3 else 1.0
            ax.plot(t, data, color=color, linewidth=lw)
